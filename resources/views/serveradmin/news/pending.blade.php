...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('panel.dash-pending-news') }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{route('serveradmin.dashboard')}}">{{ __('panel.home') }}</a></li>
                        <li class="breadcrumb-item active">{{ __('panel.dash-pending-news') }}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header d-flex">
                            <h3 class="card-title my-auto" style="font-size: 18px; font-weight: 600;">
                                {{ __('panel.dash-pending-news') }}
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip" data-placement="top" title="Total Number of Pending Newses" style="font-size: 14px; font-weight: 500;">{{ $pendingNews->count() }}</span>
                            </h3>
                        ...
                        <div class="card-body">
                            <table id="tblNews" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @foreach ($pendingNews as $key=>$news)
                                        <tr>
                                            ...
                                            <td>
                                                <input data-id="{{$news->id}}" class="toggle-class" type="checkbox" data-onstyle="success" data-offstyle="danger" data-size="small" data-toggle="toggle" data-style="slow" data-on="Approved" data-off="Pending" {{ $news->status ? '' : 'checked' }}>
                                            </td>
                                            <td class="text-center w-25">
                                                <a href="{{ route('serveradmin.newses.edit', $news->id) }}" class="btn btn-sm btn-info mt-1"><i class="nav-icon fas fa-edit" style="margin-right: 5px;"></i>{{ __('panel.news-btn-edit') }}</a>

                                                <button type="button" class="btn btn-sm btn-danger mt-1" onclick="deletePost({{ $news->id }})"><i class="nav-icon fas fa-trash-alt" style="margin-right: 5px;"></i>{{ __('panel.news-btn-delete') }}</button>
                                                <form id="delete-form-{{ $news->id }}" action="{{ route('serveradmin.newses.destroy', $news->id) }}" method="POST" style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
