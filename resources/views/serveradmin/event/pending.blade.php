...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('panel.dash-pending-event') }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{route('serveradmin.dashboard')}}">{{ __('panel.home') }}</a></li>
                        <li class="breadcrumb-item active">{{ __('panel.dash-pending-event') }}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header d-flex">
                            <h3 class="card-title my-auto" style="font-size: 18px; font-weight: 600;">
                                {{ __('panel.dash-pending-event') }}
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip" data-placement="top" title="Total Number of Pending Events" style="font-size: 14px; font-weight: 500;">{{ $pendingEvent->count() }}</span>
                            </h3>

                            {{-- <a class="btn btn-sm btn-primary ml-auto" href="{{route('superadmin.newses.create')}}"><i class="nav-icon fas fa-plus-circle" style="margin-right: 5px;"></i>{{ __('panel.news-btn-add') }}</a> --}}
                            ...
                            <table id="tblEvent" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">{{ __('panel.event-tbl-status')}}</th>
                                        <th class="text-center">{{ __('panel.event-tbl-action')}}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($pendingEvent as $key=>$event)
                                        <tr>
                                            <td class="text-center">{{ $key + 1 }}</td>
                                            <td class="w-25">{{$event->title}}</td>
                                            <td>{{ date('d-m-Y', strtotime($event->date)) }}</td>
                                            <td class="w-50 text-justify">{!! Str::limit($event->description, 300, '...') !!}</td>
                                            <td>
                                                @php
                                                ...
                                                @else
                                                    <img src="{{asset('public/assets/images/image-gallery.jpg')}}" width="150" height="100" class="img-responsive rounded" alt="image">
                                                @endif

                                                {{-- <img src="{{asset('public/storage/news_image/'.$news->image)}}" width="150" height="100" class="img-responsive rounded" alt="image"> --}}
                                            </td>
                                            <td>
                                                <input data-id="{{$event->id}}" class="toggle-class" type="checkbox" data-onstyle="success" data-offstyle="danger" data-size="small" data-toggle="toggle" data-style="slow" data-on="Approved" data-off="Pending" {{ $event->status ? '' : 'checked' }}>
                                            </td>
                                            <td class="text-center w-25">
                                                <a href="{{ route('serveradmin.events.edit', $event->id) }}" class="btn btn-sm btn-info mt-1"><i class="nav-icon fas fa-edit" style="margin-right: 5px;"></i>{{ __('panel.news-btn-edit') }}</a>

                                                <button type="button" class="btn btn-sm btn-danger mt-1" onclick="deletePost({{ $event->id }})"><i class="nav-icon fas fa-trash-alt" style="margin-right: 5px;"></i>{{ __('panel.news-btn-delete') }}</button>
                                                <form id="delete-form-{{ $event->id }}" action="{{ route('serveradmin.events.destroy', $event->id) }}" method="POST" style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
