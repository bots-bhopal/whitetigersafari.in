...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('panel.dash-pending-tenders') }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{route('serveradmin.dashboard')}}">{{ __('panel.home') }}</a></li>
                        <li class="breadcrumb-item active">{{ __('panel.dash-pending-tenders') }}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header d-flex">
                            <h3 class="card-title my-auto" style="font-size: 18px; font-weight: 600;">
                                {{ __('panel.dash-pending-tenders') }}
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip" data-placement="top" title="Total Number of Pending Tender's" style="font-size: 14px; font-weight: 500;">{{ $pendingTender->count() }}</span>
                            </h3>
                            {{-- <a class="btn btn-sm btn-primary ml-auto" href="{{route('superadmin.tender.create')}}"><i class="nav-icon fas fa-plus-circle" style="margin-right: 5px;"></i>{{ __('panel.tender-btn-add') }}</a> --}}
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="tblTender" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @foreach($pendingTender as $key=>$tender)
                                        <tr>
                                            ...
                                            <td>
                                                <input data-id="{{$tender->id}}" class="toggle-class" type="checkbox" data-onstyle="success" data-offstyle="danger" data-size="small" data-toggle="toggle" data-style="slow" data-on="Approved" data-off="Pending" {{ $tender->status ? '' : 'checked' }}>
                                            </td>
                                            <td class="text-center">
                                                <a href="{{ route('serveradmin.tender.edit', $tender->id) }}" class="btn btn-sm btn-primary mt-1" data-toggle="tooltip" data-placement="top" title="Edit" style="font-size: 14px; font-weight: 500;"><i class="far fa-edit"></i> Edit</a>

                                                <button type="button" class="btn btn-sm btn-danger mt-1" onclick="deleteTender({{ $tender->id }})" data-toggle="tooltip" data-placement="top" title="Delete" style="font-size: 14px; font-weight: 500;"><i class="fa fa-trash-alt"></i> Delete</button>
                                                <form id="delete-form-{{ $tender->id }}" action="{{ route('serveradmin.tender.destroy', $tender->id) }}" method="POST" style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>

                                                <a href="{{ route('str.tender.download', $tender->original_filename) }}" class="btn btn-sm btn-success mt-1" data-toggle="tooltip" data-placement="top" title="Download" style="font-size: 14px; font-weight: 500;"><i class="fa fa-download"></i> Download</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
...
@push('js')
    ...
    <script>
        ...
        // Toggle Switch Tender
        $('#tblTender').on('draw.dt', function(){
            $('.toggle-class').change(function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: 'changetenderstatus',
                    data: {'status': status, 'id': tender_id},
